...
<body>
    ...
    $(document).ready(function() {
        ...
        function loadMovies(category_id) {
            var getData = new FormData();
            getData.append('function', 'listmovies');
            getData.append('category_id', category_id);
            $.ajax({
                type: "POST",
        ...
        console.log("test",loadMovies(16));

        function loadCategories() {
            var getData = new FormData();
            getData.append('function', 'listcategories');
            $.ajax({
                type: "POST",
                url: 'controllers/allmovies.php',
                processData: false,
                ...
                success: function(response) {
                    if (!response.error) {
                        ...
                        response.result.forEach(element => {
                            var movieslist = loadMovies(element.id);
                            console.log(movieslist);
                            var movieliststring = "";
                            if(movieslist){
                                movieslist.forEach(movie_element => {
                                    movieliststring += `<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-4 g-4">
                                                            <div class="col">
                                                                <div class="card h-100">
                            ...
                            div_string +=
                                `<div class="tab-pane fade" id="${element.id}_pane" role="tabpanel" aria-labelledby="${element.id}-tab">No Movies Found in this Category</div>`;
                        });
